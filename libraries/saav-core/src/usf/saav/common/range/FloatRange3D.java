...
public class FloatRange3D {
	...
	/*
	@Deprecated
	void draw(PGraphics g, float size) {
		Float3 minr = getMinimum();
		Float3 maxr = getMaximum();

		g.strokeWeight(size);
		g.noFill();
		g.stroke(0, 0, 0, 255);
		g.beginShape(PConstants.LINES);
		g.vertex(minr.x, minr.y, minr.z);
		g.vertex(maxr.x, minr.y, minr.z);
		g.vertex(minr.x, maxr.y, minr.z);
		g.vertex(maxr.x, maxr.y, minr.z);
		g.vertex(minr.x, minr.y, maxr.z);
		g.vertex(maxr.x, minr.y, maxr.z);
		g.vertex(minr.x, maxr.y, maxr.z);
		g.vertex(maxr.x, maxr.y, maxr.z);
		...
		g.translate(minr.x, minr.y, minr.z);
		g.sphere(size * 0.5f);
		g.translate(-minr.x, -minr.y, -minr.z);
		g.translate(maxr.x, minr.y, minr.z);
		g.sphere(size * 0.5f);
		g.translate(-maxr.x, -minr.y, -minr.z);
		g.translate(minr.x, maxr.y, minr.z);
		g.sphere(size * 0.5f);
		g.translate(-minr.x, -maxr.y, -minr.z);
		g.translate(maxr.x, maxr.y, minr.z);
		g.sphere(size * 0.5f);
		g.translate(-maxr.x, -maxr.y, -minr.z);
		g.translate(minr.x, minr.y, maxr.z);
		g.sphere(size * 0.5f);
		g.translate(-minr.x, -minr.y, -maxr.z);
		g.translate(maxr.x, minr.y, maxr.z);
		g.sphere(size * 0.5f);
		g.translate(-maxr.x, -minr.y, -maxr.z);
		g.translate(minr.x, maxr.y, maxr.z);
		g.sphere(size * 0.5f);
		g.translate(-minr.x, -maxr.y, -maxr.z);
		g.translate(maxr.x, maxr.y, maxr.z);
		g.sphere(size * 0.5f);
		g.translate(-maxr.x, -maxr.y, -maxr.z);
