...
public class MathX {
	...
	public static double nextSmallerPowerOf2( double v ){
		return nextLargerPowerOf2((long)(v/2+1));
	}
	...
	public static double nextLargerPowerOf2( double v ){
		return nextLargerPowerOf2( (long)v );
	}
	...
	public static float map(float val, float minV, float maxV, float outMin, float outMax) {
		return (val-minV)/(maxV-minV)*(outMax-outMin) + outMin;
	}

	public static int max(int[] bins) {
		int ret = bins[0];
		for( int v : bins ) { ret = Math.max(ret, v); }
		return ret;
	}
